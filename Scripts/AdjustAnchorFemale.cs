...
// Type: ActEditor.Core.Scripts.AdjustAnchorFemale
...
namespace ActEditor.Core.Scripts
{
  public class AdjustAnchorFemale : IActScript
  {
      ...
      int[] selectedLayerIndexes)
    {
      ...
      try
      {
        AdjustAnchor.AdjustAnchors(act, new Act((MultiType) ApplicationManager.GetResource("ref_head_f.act"), new Spr()));
      }
      catch (Exception ex)
      {
    ...
    public object DisplayName => (object) "Adjust anchors (female)";
