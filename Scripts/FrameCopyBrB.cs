...
// Type: ActEditor.Core.Scripts.FrameCopyBrB
  ...
  public class FrameCopyBrB : IActScript
  {
    public object DisplayName
    {
      get
      {
        ...
        displayName.Inlines.Add((Inline) new Bold((Inline) new Run("bottom right")));
        displayName.Inlines.Add(" to ");
        displayName.Inlines.Add((Inline) new Bold((Inline) new Run("bottom")));
        displayName.Inlines.Add(" (common)");
      ...
      int[] selectedLayerIndexes)
    {
      ...
      {
        act.Commands.Begin();
        int num1 = selectedActionIndex / 8 * 8;
        for (int index1 = num1 + 1; index1 < act.NumberOfActions && index1 < num1 + 8; index1 += 2)
        {
          int num2 = index1 == num1 + 7 ? -7 : 1;
          act.Commands.FrameDeleteRange(index1 + num2, 0, act[index1 + num2].NumberOfFrames);
          for (int index2 = 0; index2 < act[index1].NumberOfFrames; ++index2)
            act.Commands.FrameCopyTo(index1, index2, index1 + num2, index2);
        }
      }
      catch (Exception ex)
      {
