...
// Type: ActEditor.Core.Scripts.ImportAnchor
  ...
  public class ImportAnchor : IActScript
  {
      ...
      int[] selectedLayerIndexes)
    {
      ...
      try
      {
        ...
        if (str == null)
          return;
        Act loadedAct = str.IsExtension(".act") ? new Act((MultiType) File.ReadAllBytes(str), new Spr()) : throw new Exception("Invalid file extension (act file supported only).");
        act.Commands.Begin();
    ...
    public object DisplayName => (object) "Set from file...";

    public string Group => "Anchors/Set anchors";

    public string InputGesture => "Ctrl-Alt-K";
