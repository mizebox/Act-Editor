...
// Type: ActEditor.Core.WPF.Dialogs.FrameInsertDialog
...
{
  public partial class FrameInsertDialog : TkWindow, IComponentConnector
  {
    ...
    public FrameInsertDialog()
      : base("Frame edit", "advanced.png")
    {
    ...
    {
      if (this.Mode == FrameInsertDialog.EditMode.None)
        return false;
      try
      {
    ...
    public void Execute(Act act, bool executeCommands = true)
    {
      switch (this.Mode)
      {
        case FrameInsertDialog.EditMode.None:
          throw new Exception("No command selected.");
        case FrameInsertDialog.EditMode.Delete:
          if (this.StartIndex < 0)
            this.StartIndex = 0;
          if (this.StartIndex >= act[this.ActionIndex].NumberOfFrames)
            this.StartIndex = act[this.ActionIndex].NumberOfFrames - 1;
          if (this.StartIndex + this.Range > act[this.ActionIndex].NumberOfFrames)
            this.Range = act[this.ActionIndex].NumberOfFrames - this.StartIndex;
          if (this.Range < 1)
            this.Range = 1;
          if (act[this.ActionIndex].NumberOfFrames - this.Range <= 0)
            throw new ArgumentException("There must be at least one frame left in the act.");
          if (!executeCommands)
            break;
          try
          {
            act.Commands.FrameDeleteRange(this.ActionIndex, this.StartIndex, this.Range);
            break;
          }
          catch (Exception ex)
          {
        ...
        case FrameInsertDialog.EditMode.Insert:
          if (GrfEditorConfiguration.ActEditorCopyFromCurrentFrame)
          {
            ...
            if (this.StartIndex >= act[this.ActionIndex].NumberOfFrames)
              throw new ArgumentException("Start index must be less than the number of actions (" + (object) act[this.ActionIndex].NumberOfFrames + ").");
          }
          if (this.Range < 1)
            this.Range = 1;
          if (this.EndIndex > act[this.ActionIndex].NumberOfFrames)
            this.EndIndex = act[this.ActionIndex].NumberOfFrames;
          if (this.EndIndex < 0)
            this.EndIndex = 0;
          if (!executeCommands)
            break;
            ...
            if (GrfEditorConfiguration.ActEditorCopyFromCurrentFrame)
            {
              for (int index = 0; index < this.Range; ++index)
              {
                if (this.EndIndex > this.StartIndex)
                  act.Commands.FrameCopyTo(this.ActionIndex, this.StartIndex, this.EndIndex);
                else
                  act.Commands.FrameCopyTo(this.ActionIndex, this.StartIndex + index, this.EndIndex);
              }
              break;
            }
            for (int index = 0; index < this.Range; ++index)
              act.Commands.FrameInsertAt(this.ActionIndex, this.EndIndex);
            break;
          }
          catch (Exception ex)
          {
        ...
        case FrameInsertDialog.EditMode.Move:
          if (this.StartIndex >= act[this.ActionIndex].NumberOfFrames)
            this.StartIndex = act[this.ActionIndex].NumberOfFrames - 1;
          if (this.EndIndex >= act[this.ActionIndex].NumberOfFrames)
            this.EndIndex = act[this.ActionIndex].NumberOfFrames - 1;
          if (this.StartIndex < 0)
            this.StartIndex = 0;
          if (this.EndIndex < 0)
            this.EndIndex = 0;
          ...
          try
          {
            act.Commands.FrameMoveRange(this.ActionIndex, this.StartIndex, this.Range, this.EndIndex);
            break;
          }
          catch (Exception ex)
          {
        ...
        case FrameInsertDialog.EditMode.Switch:
          ...
          try
          {
            act.Commands.FrameSwitchRange(this.ActionIndex, this.StartIndex, this.Range, this.EndIndex);
            break;
          }
          catch (Exception ex)
          {
        ...
        case FrameInsertDialog.EditMode.Replace:
          ...
          {
            act.Commands.BeginNoDelay();
            while (this.EndIndex + this.Range > act[this.ActionIndex].NumberOfFrames)
              act.Commands.FrameInsertAt(this.ActionIndex, act[this.ActionIndex].NumberOfFrames);
            for (int index = 0; index < this.Range; ++index)
              act.Commands.FrameReplace(this.ActionIndex, this.StartIndex, this.EndIndex + index);
            break;
          }
          catch (Exception ex)
          {
    ...
    {
      FrameInsertDialog.EditMode editMode = FrameInsertDialog.EditMode.None;
      if (sender == this._mode0)
        editMode = FrameInsertDialog.EditMode.Delete;
      else if (sender == this._mode1)
        editMode = FrameInsertDialog.EditMode.Insert;
      else if (sender == this._mode2)
        editMode = FrameInsertDialog.EditMode.Move;
      else if (sender == this._mode3)
        editMode = FrameInsertDialog.EditMode.Switch;
      else if (sender == this._mode4)
        editMode = FrameInsertDialog.EditMode.Replace;
      this.Mode = editMode;
    }

    private void _lastIndex_Click(object sender, RoutedEventArgs e) => this.EndIndex = this._act[this.ActionIndex].NumberOfFrames;
    ...
    public void InitializeComponent()
    {
      ...
      Application.LoadComponent((object) this, new Uri("/Act Editor;component/core/wpf/dialogs/frameinsertdialog.xaml", UriKind.Relative));
    }
